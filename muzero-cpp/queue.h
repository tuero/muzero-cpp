...
#ifndef MUZERO_CPP_THREAD_SAFE_QUEUE_H_
#define MUZERO_CPP_THREAD_SAFE_QUEUE_H_
...
public:
    ...
    bool Push(const T& value, absl::Time deadline) {
        ...
        if (block_new_values_) { return false; }
        while ((int)q_.size() >= max_size_) {
            if (absl::Now() > deadline || block_new_values_) { return false; }
    ...
    absl::optional<T> Pop(absl::Time deadline) {
        absl::MutexLock lock(&m_);
        while (q_.empty()) {
            if (absl::Now() > deadline || block_new_values_) { return absl::nullopt; }
            cv_.WaitWithDeadline(&m_, deadline);
        }
        T val = q_.front();
        q_.pop();
...
#endif    // MUZERO_CPP_THREAD_SAFE_QUEUE_H_
