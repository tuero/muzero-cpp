...
)
{
    ...
    while (!stop->stop_requested() && step <= config.max_training_steps) {
        ...
        if (step % config.checkpoint_interval == 0) {
            std::string checkpoint_path = device_manager->Get(0, device_id)->SaveCheckpoint(kMostRecentCheckpointStep);
        ...
        if (current_reward > max_reward) {
            ...
            std::string checkpoint_path = device_manager->Get(0, 1)->SaveCheckpoint(kBestPerformanceCheckpointStep);
            std::cout << "\33[2K\rcheckpoint saved: " << checkpoint_path << std::endl;
        }
