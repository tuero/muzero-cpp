...
VPRNetModel::VPRNetModel(const Config &config, const std::string &device)
    : device_(device),
      ...
      model_optimizer_(
          model_->parameters(),
          torch::optim::AdamOptions(config.model_config.learning_rate).weight_decay(config.model_config.l2_weight_decay)
      ),
      ...
      action_flat_size_(config.action_channels * encoded_obs_shape_.h * encoded_obs_shape_.w),
      value_loss_weight_(config.value_loss_weight)
{
...
std::vector<VPRNetModel::InferenceOutputs>
    VPRNetModel::InitialInference(std::vector<VPRNetModel::InitialInferenceInputs> &inputs)
{
    int batch_size = static_cast<int>(inputs.size());
    ...
    for (int idx = 0; idx < batch_size; ++idx) {
        auto &batch_item = inputs[static_cast<std::size_t>(idx)];
        stacked_observations[idx].copy_(
            torch::from_blob(batch_item.stacked_observation.data(), {initial_flat_size_}, options)
        );
    }
    ...
    torch::Tensor value_decoded = value_encoder_.decode(batched_output.value).to(torch::kCPU).to(torch::kDouble);
    torch::Tensor reward_decoded = reward_encoder_.decode(batched_output.reward).to(torch::kCPU).to(torch::kDouble);
    ...
    for (int batch = 0; batch < batch_size; ++batch) {
        ...
        auto value = value_decoded[batch][0].item<double>();
        auto reward = reward_decoded[batch][0].item<double>();
        ...
        std::vector<double> policy(
            batch_policy.data_ptr<double>(),
            batch_policy.data_ptr<double>() + batch_policy.numel()
        );
        torch::Tensor batch_encoded_state = batched_output.encoded_state[batch].to(torch::kCPU).to(torch::kDouble);
        Observation encoded_state(
            batch_encoded_state.data_ptr<double>(),
            batch_encoded_state.data_ptr<double>() + batch_encoded_state.numel()
        );
        outputs.push_back({.value = value, .reward = reward, .policy = policy, .encoded_state = encoded_state});
    }
    return outputs;
}

std::vector<VPRNetModel::InferenceOutputs>
    VPRNetModel::RecurrentInference(std::vector<VPRNetModel::RecurrentInferenceInputs> &inputs)
{
    ...
    for (int idx = 0; idx < batch_size; ++idx) {
        auto &batch_item = inputs[static_cast<std::size_t>(idx)];
        encoded_observations[idx].copy_(
            torch::from_blob(batch_item.encoded_state.data(), {recurrent_flat_size_}, options)
        );
        Observation encoded_action = config_.action_representation_recurrent(batch_item.action);
        actions[idx].copy_(torch::from_blob(encoded_action.data(), {(int)encoded_action.size()}, options));
    }

    // Reshape to expected size for network (batch_size, flat) -> (batch_size, c, h, w)
    encoded_observations = encoded_observations.reshape({batch_size, -1, encoded_obs_shape_.h, encoded_obs_shape_.w});
    ...
    for (int batch = 0; batch < batch_size; ++batch) {
        ...
        torch::Tensor batch_encoded_state = batched_output.encoded_state[batch].to(torch::kDouble).to(torch::kCPU);
        Observation encoded_state(
            batch_encoded_state.data_ptr<double>(),
            batch_encoded_state.data_ptr<double>() + batch_encoded_state.numel()
        );
...
VPRNetModel::LossInfo VPRNetModel::Learn(Batch &inputs)
{
    ...
    encoded_actions.reserve(static_cast<std::size_t>(batch_size * num_steps * action_flat_size_));
    ...
    torch::Tensor stacked_observations =
        torch::from_blob(inputs.stacked_observations.data(), {batch_size, initial_flat_size_}, options_float)
            .to(torch_device_);
    torch::Tensor actions =
        torch::from_blob(encoded_actions.data(), {batch_size, num_steps, action_flat_size_}, options_float)
            .to(torch_device_);
    torch::Tensor target_values = torch::from_blob(inputs.target_values.data(), {batch_size, num_steps}, options_double)
                                      .to(torch_device_, torch::kFloat);
    torch::Tensor target_rewards =
        torch::from_blob(inputs.target_rewards.data(), {batch_size, num_steps}, options_double)
            .to(torch_device_, torch::kFloat);
    torch::Tensor target_policies =
        torch::from_blob(inputs.target_policies.data(), {batch_size, num_steps, num_actions}, options_double)
            .to(torch_device_, torch::kFloat);
    torch::Tensor gradient_scale = torch::from_blob(inputs.gradient_scale.data(), {batch_size, 1}, options_double)
                                       .to(torch_device_, torch::kFloat);
    torch::Tensor is_priorities =
        torch::from_blob(inputs.priorities.data(), {batch_size, 1}, options_double).to(torch_device_, torch::kFloat);
    ...
    model::InferenceOutput model_init_output = model_->initial_inference(stacked_observations);
    LossOutput loss_init_output = model_->loss(
        model_init_output.value,
        model_init_output.reward,
        model_init_output.policy_logits,
        target_values_encoded.index({Slice(), 0}),
        target_rewards_encoded.index({Slice(), 0}),
        target_policies.index({Slice(), 0})
    );
    torch::Tensor value_loss = loss_init_output.value_loss;
    torch::Tensor reward_loss = torch::zeros_like(loss_init_output.reward_loss);
    torch::Tensor policy_loss = loss_init_output.policy_loss;

    // Get updated errors for updating the prioritized replay buffer
    // |search value - observed n_step value|
    torch::Tensor error_values = (value_encoder_.decode(model_init_output.value).to(torch::kCPU)
                                  - target_values.index({Slice(), Slice(0, 1)}).to(torch::kCPU).to(torch::kDouble))
                                     .squeeze(0);
    std::vector<double> errors(error_values.data_ptr<double>(), error_values.data_ptr<double>() + error_values.numel());

    // Generate recurrent predictions
    torch::Tensor prev_state = model_init_output.encoded_state;
    for (int step = 1; step < num_steps; ++step) {
        model::InferenceOutput model_output = model_->recurrent_inference(prev_state, actions.index({Slice(), step}));
        ...
        LossOutput loss_output = model_->loss(
            ...
            target_values_encoded.index({Slice(), step}),
            target_rewards_encoded.index({Slice(), step}),
    ...
    return {
        ...
        .value = value_loss.mean().item<double>(),
        .policy = policy_loss.mean().item<double>(),
        .reward = reward_loss.mean().item<double>(),
