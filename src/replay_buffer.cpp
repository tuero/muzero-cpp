...
PrioritizedReplayBuffer::PrioritizedReplayBuffer(const Config &config, int max_size, const std::string &name)
...
auto PrioritizedReplayBuffer::can_sample() const -> bool
{
    return tree_.get_size() >= min_sample_size_;
}

// Get the number of stored items.
auto PrioritizedReplayBuffer::size() const -> int
{
...
void PrioritizedReplayBuffer::save_game_history(const GameHistory &game_history)
{
    ...
    for (std::size_t i = 0; i < game_history.root_values.size(); ++i) {
        errors.push_back(
            game_history.root_values[i] - game_history.compute_target_value(static_cast<int>(i), td_steps_, discount_)
        );
    }
...
void PrioritizedReplayBuffer::update_history_priorities(const std::vector<int> &indices, std::vector<double> &errors)
{
    absl::MutexLock lock(&m_);
...
auto PrioritizedReplayBuffer::sample_game(std::mt19937 &rng) -> GameSample
{
    absl::MutexLock lock(&m_);
    std::uniform_int_distribution<> uniform_dist(1, tree_.get_num_histories());
    int history_id = uniform_dist(rng);
    return {.id = history_id, .game_history = tree_.get_history(history_id)};
}

// Get a batched sample from the replay buffer
auto PrioritizedReplayBuffer::sample(std::mt19937 &rng, int batch_size) -> Batch
{
    absl::MutexLock lock(&m_);
    // Update beta
    ...
    for (int i = 0; i < batch_size; ++i) {
        ...
        samples.stacked_observations
            .insert(samples.stacked_observations.end(), current_stacked_obs.begin(), current_stacked_obs.end());
    }

    // Get importance sampling weights
    for (std::size_t i = 0; i < static_cast<std::size_t>(batch_size); ++i) {
        samples.priorities[i] = std::pow(tree_.get_size() * samples.priorities[i] / tree_.total_priority(), -beta_);
    }
    double max_value = *std::ranges::max_element(samples.priorities);
    for (std::size_t i = 0; i < static_cast<std::size_t>(batch_size); ++i) {
