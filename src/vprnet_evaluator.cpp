      ...
      queue_recurrent_(batch_size_recurrent * recurrent_inference_threads * 4)
{
    ...
    if (batch_size_initial_ <= 1) {
        initial_inference_threads = 0;
    }
    if (batch_size_recurrent_ <= 1) {
        recurrent_inference_threads = 0;
    }

    // Reserve space for each type of inference
    initial_inference_threads_.reserve(static_cast<std::size_t>(initial_inference_threads));
    for (int i = 0; i < initial_inference_threads; ++i) {
        initial_inference_threads_.emplace_back([this]() { this->InitialInferenceRunner(); });
    }
    recurrent_inference_threads_.reserve(static_cast<std::size_t>(recurrent_inference_threads));
    for (int i = 0; i < initial_inference_threads; ++i) {
...
auto VPRNetEvaluator::InitialInference(const Observation &stacked_observations) -> VPRNetModel::InferenceOutputs
{
    ...
    if (batch_size_initial_ <= 1) {
        ...
        queue_initial_.Push(QueueItemInitial{.inputs = inputs, .prom = &prom});
        outputs = fut.get();
    }
    return outputs;
}
...
auto VPRNetEvaluator::RecurrentInference(const Observation &hidden_state, Action action)
    -> VPRNetModel::InferenceOutputs
{
    VPRNetModel::RecurrentInferenceInputs inputs = {.action = action, .encoded_state = hidden_state};
    VPRNetModel::InferenceOutputs outputs;
    // Get output directly, or insert into queue and wait for batched input to be sent back
    if (batch_size_recurrent_ <= 1) {
        std::vector inference_inputs{inputs};
        ...
        std::promise<VPRNetModel::InferenceOutputs> prom;
        std::future<VPRNetModel::InferenceOutputs> fut = prom.get_future();
        queue_recurrent_.Push(QueueItemRecurrent{.inputs = inputs, .prom = &prom});
        outputs = fut.get();
    }
    return outputs;
}
...
void VPRNetEvaluator::InitialInferenceRunner()
{
    ...
    inputs.reserve(static_cast<std::size_t>(batch_size_initial_));
    promises.reserve(static_cast<std::size_t>(batch_size_initial_));
    while (!stop_token_.stop_requested()) {
        ...
        std::vector<VPRNetModel::InferenceOutputs> outputs =
            device_manager_.Get(static_cast<int>(inputs.size()))->InitialInference(inputs);
...
void VPRNetEvaluator::RecurrentInferenceRunner()
{
    ...
    std::vector<std::promise<VPRNetModel::InferenceOutputs> *> promises;
    inputs.reserve(static_cast<std::size_t>(batch_size_recurrent_));
    promises.reserve(static_cast<std::size_t>(batch_size_recurrent_));
    while (!stop_token_.stop_requested()) {
        {
            // Only one thread at a time should be listening to the queue to maximize
            // batch size and minimize latency.
        ...
        std::vector<VPRNetModel::InferenceOutputs> outputs =
            device_manager_.Get(static_cast<int>(inputs.size()))->RecurrentInference(inputs);
        // Send result to waiting promise/future connections
        for (std::size_t i = 0; i < promises.size(); ++i) {
            promises[i]->set_value(outputs[i]);
        }
