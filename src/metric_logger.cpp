...
void metric_logger(const Config &config, std::shared_ptr<SharedStats> shared_stats, std::shared_ptr<StopToken> stop)
{
    ...
    TensorBoardLogger writer(
        absl::StrCat(config.path, "/metrics/", "tfevents.pb"),
        TensorBoardLoggerOptions{}.resume(config.resume)
    );
    ...
    for (int step = shared_stats->get_logging_step(); !stop->stop_requested(); ++step) {
        ...
        if (step == 1) {
            absl::SleepFor(absl::Milliseconds(2.5 * MS_PER_SEC));
        }
        // Get stats
        shared_stats->set_metric_logger();
        ...
        writer.add_scalar("2.Workers/4.Reanalyze_games", step, (double)stats.num_reanalyze_games);
        writer.add_scalar("2.Workers/5.Reanalyze_steps", step, (double)stats.num_reanalyze_steps);
        writer.add_scalar(
            "2.Workers/6.Training_per_selfplay_step_ratio",
            step,
        ...
        std::cout << "\33[2K\r";
        std::cout << std::format(
            "Train step: {}/{}, Selfplay games: {}, Reanalyze "
            "trajs: {}, Train speed: {:.2f} s/s, Selfplay speed {:.2f} steps/s",
            stats.training_step,
            config.max_training_steps,
            stats.num_played_games,
            stats.num_reanalyze_games,
            stats.train_speed,
        ...
        // Sleep
        absl::SleepFor(absl::Milliseconds(MS_PER_SEC));
    }
}
