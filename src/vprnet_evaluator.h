...
#ifndef MUZERO_VPRNET_EVALUATOR_H_
#define MUZERO_VPRNET_EVALUATOR_H_
...
public:
    ...
    virtual auto InitialInference(const Observation &stacked_observations) -> model::VPRNetModel::InferenceOutputs = 0;
    virtual auto RecurrentInference(const Observation &hidden_state, Action action)
        -> model::VPRNetModel::InferenceOutputs = 0;
};
...
public:
     ...
     */
    explicit VPRNetEvaluator(
        DeviceManager *device_manager,
        int batch_size_initial,
        int initial_inference_threads,
        int batch_size_recurrent,
        int recurrent_inference_threads
    );
    ~VPRNetEvaluator() override;
     ...
     */
    auto InitialInference(const Observation &stacked_observations) -> model::VPRNetModel::InferenceOutputs override;
     ...
     * @return The value, reward, policy, and encoded state during initial inference
     */
    auto RecurrentInference(const Observation &hidden_state, Action action)
        -> model::VPRNetModel::InferenceOutputs override;
...
private:
    ...
    struct QueueItemInitial {
        model::VPRNetModel::InitialInferenceInputs inputs;
        std::promise<model::VPRNetModel::InferenceOutputs> *prom;
    };
    // Struct for holding promised value for recurrent inference queries
    struct QueueItemRecurrent {
        model::VPRNetModel::RecurrentInferenceInputs inputs;
        std::promise<model::VPRNetModel::InferenceOutputs> *prom;
    };
    ...
    StopToken stop_token_;                                    // Stop token flag to signal to quit
...
#endif    // MUZERO_VPRNET_EVALUATOR_H_
