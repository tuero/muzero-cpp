#ifndef MUZERO_MODELS_H_
#define MUZERO_MODELS_H_
...
public:
    ...
    static auto encoded_state_shape(const ObservationShape &observation_shape, bool downsample) -> ObservationShape;
...
public:
    ...
    auto forward(torch::Tensor x) -> DynamicsOutput;

private:
    int reward_mlp_input_size_;              // Flat input size for reward sub-module
    layers::ResidualHead resnet_head_;       // Resnet head
    torch::nn::Conv2d conv1x1_reward_;       // Conv pass before passing to reward mlp
    layers::MLP reward_mlp_;                 // MLP network for predicted reward
...
public:
    ...
    auto forward(torch::Tensor x) -> PredictionOutput;
...
private:
    ...
    layers::MLP policy_mlp_;                 // MLP network for predicted policy
    layers::MLP value_mlp_;                  // MLP network for predicted value
...
public:
    MuzeroNetworkImpl(const Config &config);
     ...
     */
    auto initial_inference(torch::Tensor observation) -> InferenceOutput;
     ...
     */
    auto recurrent_inference(torch::Tensor encoded_state, torch::Tensor action) -> InferenceOutput;
...
private:
    ...
    ObservationShape encoded_observation_shape_;    // Input shape to dynamics/prediction network
