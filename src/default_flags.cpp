...
ABSL_FLAG(std::string, export_path, "/opt/muzero-cpp/", "Base path to store all checkpoints and metrics");
ABSL_FLAG(std::string, model_path, "model.json", "Base of model json config");
...
ABSL_FLAG(int, testing_checkpoint, -1, "Testing checkpoint to load: -1 for most recent, -2 for best performing");
...
ABSL_FLAG(double, dirichlet_epsilon, 0.25, "The fractional component of weighted sum for the Dirichlet noise");
...
ABSL_FLAG(int, max_history_len, -1, "Maximum history length before sending to replay buffer. Use -1 to ignore.");
...
auto initial_config() -> Config
{
    ...
    config.path = absl::GetFlag(FLAGS_export_path);
    ...
    // Model
    std::ifstream model_ifs(absl::GetFlag(FLAGS_model_path));
    auto model_config_json = nlohmann::json::parse(model_ifs);
    config.model_config = network_config_from_json(model_config_json);
