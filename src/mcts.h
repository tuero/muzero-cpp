#ifndef MUZERO_MCTS_H_
#define MUZERO_MCTS_H_
...
struct MCTSNode {
    ...
    Player to_play = InvalidPlayer;    // Current player to play at this node
    Action action = InvalidAction;     // Action taken to get to this node from the previous state
    ...
    Observation encoded_state;         // Hidden state which the nodes represents

    MCTSNode(Action a, double p)
        : action(a), prior(p)
    {}
    ...
    void expand(
        ...
        const Observation &encoded_state
    );
...
public:
    ...
    MinMaxStats()
        : init_minimum_(INF_D), init_maximum_(NINF_D), minimum_(INF_D), maximum_(NINF_D) {};
     ...
     */
    [[nodiscard]] auto normalize(double value) const -> double;
...
public:
     ...
     */
    MCTS(const Config &config, int seed, std::shared_ptr<Evaluator> vpr_eval);
    MCTS() = delete;
     ...
     */
    MCTSReturn
        run(const Observation &stacked_observation,
     ...
     */
    [[nodiscard]] auto select_child(MCTSNode *node) -> MCTSNode *;
     ...
     */
    [[nodiscard]] auto ucb_score(MCTSNode *parent, MCTSNode *child) -> double;
     ...
     */
    void backpropogate(std::vector<MCTSNode *> &search_path, double value, Player to_play);
...
private:
    ...
    std::vector<Action> action_space_;       // List of all possible actions
    std::shared_ptr<Evaluator> vpr_eval_;    // Muzero network evaluator for inference
};

}    // namespace muzero::algorithm

#endif    // MUZERO_MCTS_H_
