#ifndef MUZERO_ABSTRACT_GAME_H_
#define MUZERO_ABSTRACT_GAME_H_
...
public:
     ...
     */
    [[nodiscard]] virtual Player to_play() const = 0;
     ...
     */
    [[nodiscard]] virtual std::vector<Action> legal_actions() const = 0;
     ...
     */
    [[nodiscard]] virtual Action expert_action() = 0;

    /**
     * Returns a legal action given by human input.
     * @returns An action which is legal
     */
    [[nodiscard]] virtual Action human_to_action() = 0;
     ...
     */
    [[nodiscard]] virtual std::string action_to_string(Action action) const = 0;
};

// Factory to create instances of the abstract game
template <typename T>
auto game_factory(int seed) -> std::unique_ptr<AbstractGame>
{
